...
if __name__ == "__main__":
    ...
    for scene_idx in range(5):
        ...
        with open("../learning/annotations/masks/info.pkl", "rb") as f:
        ...
        meta_list = [ 
            ...
            dict(
                ckpt = "./pretrained_models/diffusion_pytorch_model.bin",
                input_image = f"../learning/inference/lsun-bedroom-scenes/lsun-bedroom-scenes_1_{scene_idx}.png",
                prompt = f"a bedroom with {inp_prompts[i]} on the bed",
                images = [ f'../learning/inference/inpainting-content/inpainting-content_{inp_extensions[i]}.png'],
                locations = [ [item/512 for item in list(mask_info.values())[scene_idx]] ], # mask will be derived from box 
                save_folder_name=str(os.path.join(str(scene_idx), inp_extensions[i]))
            )
